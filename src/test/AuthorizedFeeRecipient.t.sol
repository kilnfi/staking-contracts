...
import "../contracts/AuthorizedFeeRecipient.sol";
...
contract AuthorizedFeeRecipientTest {
    Vm internal vm = Vm(0x7109709ECfa91a80626fF3989D68f67F5b1DD12D);
    AuthorizedFeeRecipient internal feeRecipient;
    IFeeDispatcher internal dispatcher;
    address internal stakingContract = address(789);

    address internal bob = address(123);
    address internal alice = address(456);

    function setUp() external {
        feeRecipient = new AuthorizedFeeRecipient();
        dispatcher = IFeeDispatcher(address(new DispatcherMock()));
    }

    function _init(bytes32 _publicKeyRoot) internal {
        vm.prank(stakingContract); // Simulating the msg.sender for to mimic real call conditions
    ...
    function testCallAndDispatch_asRandom(uint256 _amount, bytes32 _publicKeyRoot) external {
        _init(_publicKeyRoot);
        address receiver = uint256(_publicKeyRoot) % 2 == 0 ? bob : alice;
